...
extension OpportunityViewModel {
...
protocol OpportunityViewModelDelegate: AnyObject {
...
final class OpportunityViewModel {
    ...
    weak var delegate: OpportunityViewModelDelegate?
    ...
    private func fetchPhotos() {
        showLoading()
        networkManager.request(endpoint: .opportunity(page: href, filter: filter), type: RoverModel.self) { [weak self] (result) in
            self?.hideLoading()
            switch result {
...
extension OpportunityViewModel: OpportunityViewModelProtocol {
    ...
    func filterFetch(_ index: Int) {
        href = "1"
        allPhotos.removeAll()
        let camera = Filter.curiosity.cameras()[index].rawValue
